<script lang="ts">
	...
	// Fed the resolved month by the streaming wrapper (+page.svelte). `filter` is
	// owned by the wrapper (bind:) so the sport lens survives prev/next month
	// switches — each switch re-streams the month and remounts this component.
	export let calendar: Awaited<PageData['calendar']>;
